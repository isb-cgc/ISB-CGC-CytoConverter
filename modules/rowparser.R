...
rowparse <- function(
    ...
    if (
        ...
        if (
            ...
            if ( !grepl("^[XY]+", Cyto_sample[2]) ) {
              ...
              if ( sexstimate==F & guess_q==F & grepl("\\?",Cyto_sample[2]) & length_XY<3 ){
                ...
                Dump_table <- rbind(Dump_table, c(Con_data, "Warning in ambiguous sex chromosome count. Default of XX set"))
              }
            } else {
              ...
              ##if sexstimate is false and guess_q is false and there is only one sex chromosome but several ? marks, set value =2
               if (sexstimate==F & guess_q==F & (xcount+ycount) < 2 & grepl("\\?",Cyto_sample[2]) & !any(grepl("Y",Cyto_sample[-2]))){
                if (ycount > 0)
                {
                  xcount <- 1
                  ycount <- 1
                  Dump_table <- rbind(Dump_table, c(Con_data, "Warning in ambiguous sex chromosome count. Default of XY set"))
        ...
        for (j in startcol:length(Cyto_sample)) {
            ...
            if (
              ...
              (grepl("^\\+[[:digit:]]+c*$", Cyto_sample[j]) |
               ...
               grepl("\\+Y", Cyto_sample[j])) && ((guess_q == T ) | (!grepl("\\?",Cyto_sample[j]))) |
                ...
                if (is.character(inc_table)) {
                   ...
                   #empty?
                    ...
                    if (nrow(temp_table) > 0) {
                        ...
                        if (
                          ((any(grepl("del", temp_table[, 4])&!grepl("^del", temp_table[, 4])&!grepl("multi[[:digit:]]*del",temp_table[,4]))) | (any(grepl("add", temp_table[, 4])&!grepl("^add", temp_table[, 4])&!grepl("multi[[:digit:]]*add",temp_table[,4]))))
                        ) {
                            if (
                              nrow(temp_table)>length(union(((intersect(grep("del", temp_table[, 4]), intersect(grep("^del", temp_table[, 4],invert=T),grep("multi[[:digit:]]*del",temp_table[,4],invert=T))))) ,(intersect(grep("add", temp_table[, 4]), intersect(grep("^add", temp_table[, 4],invert=T),grep("multi[[:digit:]]*add",temp_table[,4],invert=T))))))
                            ) {
        ...
        if (any(is.na(val))) {
            ...
            if (!is.null(val_remainder)) {
                ...
                if (val_remainder == 0) {
                  ...
                  guess==T & (any(diffval< (-14))| (any((diffval<31&diffval>14)))|any((diffval<53&diffval>38))|any((diffval>56 &(floor(val_divider)*22+3<diffval) & (ceiling(val_divider)*23-3>diffval) )))
                ) {
                    # clone_chrom_tracker <- rep(1:22, 2)
                    if (length(diffval) > 1) {
        ...
        if (forMtn == T & (xcount + ycount) == 0) {
            ...
            if (
              (idealTotal* (ploidy-2)/2 + idealTotal) != (count_before_extras) |( (count_before_extras)+xdel+ydel > (idealTotal* (ploidy-2)/2 + idealTotal) & (xdel+ydel) > 0 )
            ) {
                if (ploidy > 2) {
                ...
                if (constitutionalcount > 0) {
                    ...
                    if (
                      (count_before_extras)+xdel+ydel > idealTotal & (xdel+ydel) > 0
                    ) {

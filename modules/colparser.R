...
##take into acc same chrom insertion
##add cen into here (pter qter analogue)
...
colparse <- function(
    ...
    if ((length(test) > 1 | any(grepl("p|q",temp))| grepl("(9;22)|(22;9)",paste(unlist(temp),collapse=';',sep=";")) |(!is.null(regtranschrom) && any(grepl(regtranschrom, names(transloctable))) ) )  & (!any(grepl("\\?|\\~", temp))  ) & !(guess_q==F & grepl("\\?",Cyto_sample[coln])) ) {
        ...
        repeat {
            ...
            if (
              (((lengthcount * 2)-1)) == length(temp) || if(length(temp) > (lengthcount*2-1)){
                ...
                if (grepl("(t|ins)\\(", derMods[(lengthcount * 2-1)]) ) {
                    ...
                    transchrom <- stringr::str_extract(
                      Cyto_sample[coln], gsub("\\?","\\\\?",paste(gsub("\\+","\\\\+",gsub("\\(","\\\\(",derMods[(lengthcount*2-1)])),"\\)\\(.+?\\)",sep=''))
                    )
            ...
            if (grepl("ins\\(", derMods[(lengthcount * 2-1)])) {
                ...
                if (
                    ...
                    splitloc <- stringr::str_locate_all(temp[[lengthcount * 2]], "p|q")[[1]][, 1][2]
                    ...
                    if (!is.na(stringr::str_length(temp[[(lengthcount * 2)]][i]))) {
                        ...
                        if (
                            ...
                            for (j in 1:length(longform_table)) {
                                stringdx <- stringr::str_locate_all(pattern = "p|q", longform_table[[j]])
